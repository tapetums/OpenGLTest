...
public:
    Susie();
    explicit Susie(LPCWSTR spi_path);
    virtual ~Susie();

    bool __stdcall Load(LPCWSTR spi_path);
    bool __stdcall Free();

    LPCWSTR __stdcall SpiPath() const;

    SUSIE_API GetPluginInfo(int32_t infono, LPWSTR buf, int32_t buflen);
    SUSIE_API IsSupported(LPCWSTR filename, void* dw);

    SUSIE_API GetPictureInfo(LPCWSTR buf, size_t len, SPI_FLAG flag, SusiePictureInfo* lpInfo);
    SUSIE_API GetPicture(LPCWSTR buf, size_t len, SPI_FLAG flag, HANDLE* pHBInfo, HANDLE* pHBm, SUSIE_PROGRESS progressCallback, intptr_t lData);
    SUSIE_API GetPreview(LPCWSTR buf, size_t len, SPI_FLAG flag, HANDLE* pHBInfo, HANDLE* pHBm, SUSIE_PROGRESS progressCallback, intptr_t lData);

    SUSIE_API GetArchiveInfo(LPCWSTR buf, size_t len, SPI_FLAG flag, HLOCAL* lphInf);
    SUSIE_API GetFileInfo(LPCWSTR buf, size_t len, LPCWSTR filename, SPI_FLAG flag, SusieFileInfoW* lpInfo);
    SUSIE_API GetFile(LPCWSTR src, size_t len, LPWSTR dest, SPI_FLAG flag, SUSIE_PROGRESS progressCallback, intptr_t lData);

    SUSIE_API ConfigurationDlg(HWND parent, SPI_FNC_CODE fnc);

protected:
    WCHAR                m_spi_path[MAX_PATH];
...
public:
    ...
    Susie* __stdcall Append(LPCWSTR spi_path);
    void   __stdcall Remove(LPCWSTR spi_path);

    size_t __stdcall SpiCount() const;
    Susie* __stdcall GetAt(size_t index) const;
    Susie* __stdcall QueryAvailableSpi(LPCWSTR filename, void* dw) const;
